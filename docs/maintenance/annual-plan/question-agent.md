...
보기는 백엔드 표준 코드 표에서 만들어져 코드에서 문구를 바꿀 수 없습니다. 조회에
실패했을 때 쓰는 예비 보기 목록만 코드에 있습니다.
...
저장된 계획을 불러올 때 회차·강사·시간이 채워지는 규칙입니다.
...
법정 시간이 없는 교육의 시간을 법령 원문에서 뽑습니다.
...
설문 카드에 붙는 과거 실적 안내로 무엇을 보여줄지(최다 평가방법, 과거 강사 이름)입니다.
...
교육 목록 표, 설문 카드, 확정 요약의 머리말과 안내문입니다.

- 자리: `src/tabs/annual_plan/intake/agent.py`의 렌더 함수들
- 함께 볼 곳: 질문 카드의 화면 표시 약속(`:::q`, [API 계약](../../reference/api.md)) · 화면 문구를 고정하는 단위 테스트
- 테스트: 단위 테스트

**보기 버튼 답 처리 방식** (코드)

보기 버튼을 누르면 화면이 정해진 모양의 답 텍스트를 보내고, 코드가 그 모양을 읽어 바로
답으로 기록합니다. 모양이 다르면 일반 발화로 처리됩니다.

- 자리: `src/tabs/annual_plan/intake/agent.py`의 카드 답 해석
- 함께 볼 곳: 답 텍스트의 모양은 화면과의 약속(바꾸면 화면도 함께)
- 테스트: 단위 테스트

**수시 교육 기본 편성 여부** (코드)

수시(발생 시) 교육을 기본으로 계획에 넣을지 뺄지입니다.
...
| 수정 위치 | 내용 |
|---|---|
| `src/tabs/annual_plan/intake/agent.py` | 질문 문구·보기·기본값·값을 함께 받아야 하는 보기 표시(화면에 보이는 부분 전체)와, 계획 범위 답에 따른 자율 교육 제외·단계 생략 |
...
보기 일부는 질문 정의(코드)에 고정되어 있지 않고 실행 시점에 만들어집니다. 교육방법 보기는
백엔드 표준 코드 목록에서, 강사 보기는 부서 담당자 이름으로 조립되고, 시간이 확인되지
않는 교육의 처리 질문은 결측 검사가 추가합니다. 이런 보기의 문구는 질문 정의에서 바꿀 수
없습니다.
