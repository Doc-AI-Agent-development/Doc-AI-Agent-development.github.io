...
**설문 카드의 질문 문구·보기·기본값·값을 함께 받아야 하는 보기 표시** (코드)

- 자리: `src/tabs/annual_plan/intake/agent.py`의 질문 정의
- 함께 볼 곳: 아래 "설문 질문·보기 수정" 절(여러 파일이 한 벌)
- 테스트: 단위 테스트 + 실제 대화

**교육방법 보기의 출처(백엔드 표준 코드)와 조회 실패 시 예비 보기** (코드)
...
- 함께 볼 곳: 질문 카드 블록의 화면 계약(`:::q`, [API 계약](../../reference/api.md)) · 화면 문구를 고정하는 단위 테스트
- 테스트: 단위 테스트

**화면 버튼 답이 들어오는 정해진 형식** (코드)
