...
|---|---|---|---|---|
| 설문 카드의 질문 문구·보기·기본값·값이 함께 필요한 보기 표시 | 코드 | `src/tabs/annual_plan/intake/agent.py`의 질문 정의 | 아래 "설문 질문·보기 수정" 절 — 여러 파일이 한 벌 | 단위 테스트 + 실제 대화 |
| 교육방법 보기가 어디서 오는지(백엔드 표준 코드)와 조회 실패 시 폴백 보기 | 코드 | `src/tabs/annual_plan/intake/agent.py`의 진입부와 질문 정의 | 편집 프롬프트의 표준 라벨 목록 | 실서버 공통코드와 대조 |
| 결재가 끝난 계획을 막는 기준(결재 단계 코드) | 코드 | `src/tabs/annual_plan/intake/agent.py`의 진입부 | — | 결재가 완료된 부서로 실제 진입 시도(단위 테스트는 허용 쪽만 고정) |
| 저장된 계획을 불러올 때 값이 채워지는 규칙(회차·강사·시간 기준 완화) | 코드 | `src/tabs/annual_plan/plan_table/restore.py` | 복원 뒤 붙는 설문 틀과 결측 검사(`intake/agent.py` 진입부) | 단위 테스트 · 실서버에서 저장된 계획 불러오기 |
| 일괄 생성이 쓰는 답안 | 코드 | `src/tabs/annual_plan/intake/auto.py` | 설문 보기와 짝 — 보기를 바꾸면 함께 | 단위 테스트 |
| 법령 원문에서 연간 교육시간을 뽑는 기준 | 프롬프트 | `src/tabs/annual_plan/intake/prompts/extract_hours/` | — | 시간이 없는 교육이 있는 부서로 실제 대화 |
| 설문 카드에 붙는 과거 실적 힌트(무엇을 보여줄지) | 코드 | `src/tabs/annual_plan/intake/agent.py`의 힌트 함수 | — | 실적이 있는 부서로 실제 대화 |
| 교육 목록 표·설문 카드·확정 요약의 머리말과 안내문 | 코드 | `src/tabs/annual_plan/intake/agent.py`의 렌더 함수들 | 질문 카드 블록의 화면 계약(`:::q` — [API 계약](../../reference/api.md)) · 렌더 고정 단위 테스트 | 단위 테스트 |
| 화면 버튼으로 들어오는 답의 기계 형식 | 코드 | `src/tabs/annual_plan/intake/agent.py`의 카드 답 해석 | 프론트 계약 — 형식을 바꾸면 화면도 | 단위 테스트 |
| 수시 교육을 기본으로 넣을지 뺄지 | 코드 | `src/tabs/annual_plan/intake/agent.py`의 목록 조립 | 구상 에이전트의 수시 회차 골격 | 단위 테스트 |
