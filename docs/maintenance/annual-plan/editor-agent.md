...
편집 에이전트가 맡는 기능(발화를 값으로 옮기는 규칙, 반영 보고, 뭉뚱그린 지칭의 범위,
강사 이름 표기)을
...
발화에서 시간(회당·연간), 회차 수, 교육방법·평가방법, 강사, 수시 편성, 메모, 질문 답,
진행 의사를 읽는 규칙 전부가 한 프롬프트에 있습니다.
...
**뭉뚱그린 지칭의 범위 기준** (프롬프트 + 코드)

"전부"·"나머지" 같은 말이 화면의 어느 목록까지 가리키는지입니다.
...
직책 글자 대신 등록된 실명으로 적고, 등록에 없는 지칭은 되묻습니다.
...
무엇을 바뀐 값으로 볼지의 판정과, "✅ 반영: …"으로 알리는 문구입니다.
...
LLM이 빠뜨린 교육은 바꾸지 않고, 계획에 없는 교육 번호는 버리고, 범위 밖 값(13월 등)은
그 칸만 무시하며, 회차 시간과 연간 시간이 어긋나지 않게 맞춥니다.

- 자리: `src/tabs/annual_plan/editor/projection.py`
- 함께 볼 곳: 위 프롬프트
- 테스트: 단위 테스트

**결정 문서 칸 설명** (코드의 필드 설명)
