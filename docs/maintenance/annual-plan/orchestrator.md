...
**발화가 작업인지 질문인지 범위 밖인지 가르는 기준** (프롬프트)

제안형 표현의 처리, 설문 중 결정과 물음의 구분, 질문과 변경이 섞인 발화의 처리가 전부
이 프롬프트에 있습니다.

- 자리: `src/tabs/annual_plan/router/prompts/classify/`
- 함께 볼 곳: 분류 결과 모델(`src/schemas/dialog.py`). 분류가 새 값을 내게 하려면 모델에 칸부터 추가
