...
**응대 규칙(무엇을 기준으로 답하고, 어떤 절차를 안내하지 않는지)** (프롬프트)

- 자리: `src/tabs/annual_plan/inquiry/prompts/annual/`
- 함께 볼 곳: 상태 요약의 블록 이름을 프롬프트가 부름(요약 형식을 바꾸면 함께 고침)
- 테스트: 실 LLM 배터리

**조회 도구의 안내문(LLM이 언제 어떤 인자로 부르는지)** (도구 설명)
