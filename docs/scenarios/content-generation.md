...
```mermaid
flowchart TD
    CMP["컴플라이언스<br/>교육 기준·필수항목 조회"] --> PRE["교육자료 전처리<br/>교육 자료 구성"]
    PRE --> PLAN["교육 컨텐츠 생성<br/>발표 구성안 작성"]
    PLAN --> CK{"관리자 승인"}
    CK -->|"수정"| PLAN
    CK -->|"승인"| GEN["교육 컨텐츠 생성<br/>발표 자료 생성"]
    CK -->|"승인"| EXAM["시험 문제 생성<br/>시험·설문 생성"]
    GEN --> CHK["요건 검사<br/>근거·요건 검증"]
    EXAM --> CHK
...
| 구성안 | 교육 컨텐츠 생성 | 교육 자료 | 구성안(`plan`) | `llm` |
| 발표 자료 | 교육 컨텐츠 생성 | 구성안, 교육 자료 | 발표 자료(`presentation`) | `llm` |
| 시험·설문 | 시험 문제 생성 | 구성안, 교육 자료 | 시험(`exam`), 설문(`survey`) | `llm` |
