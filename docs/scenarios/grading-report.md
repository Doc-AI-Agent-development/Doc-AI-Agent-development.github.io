...
1. **채점** — [시험 문제 생성 에이전트](../agents/exam_generation.md)가 주관식 답안을 채점합니다. 객관식·OX는 백엔드 규칙으로 자동 채점하므로 이 단계에서 건너뜁니다. 합불·총점 판정도 백엔드가 합니다.
...
| 단계 | 에이전트 | 입력 | 출력 | 도구 |
| :-- | :-- | :-- | :-- | :-- |
| 채점 | 시험 문제 생성 | 응시 답안, 문항 정의 | 채점 결과(`grading`) | `llm` |
| 결과보고서 | 교육평가 및 요약 | 백엔드 통계, 미이수자, 합불 | 결과보고서 초안(`resultReport`) | `llm` |
| 강의평가 종합 | 교육평가 및 요약 | 수강자 후기 | 강의평가 종합(`reviewSummary`) | `llm` |
