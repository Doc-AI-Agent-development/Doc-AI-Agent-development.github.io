...
[편집 에이전트](../annual-plan/editor-agent.md)에서 다룹니다.
...
텍스트입니다. 턴 내부의 도구 호출 기록은 교육자료 생성 탭에서는 저장되지 않아 이전 턴의
조회 결과가 필요하면 다시 조회하고, 연간 교육계획 탭의 조회 응대는 조회 결과를 세션
상태에 누적해 다음 턴에 다시 실습니다([상태와 세션](./state.md)의 체크포인트 절).
