...
워크플로의 단계는 전담 에이전트가 하나씩 맡습니다. 단계 순서가 곧 에이전트 순서입니다.

```mermaid
flowchart LR
  A["질문<br/>에이전트"] --> B["구상<br/>에이전트"]
  B --> C["계획 작성<br/>에이전트"]
  C --> D["요건 검사<br/>에이전트"]
```
...
| 에이전트 | 맡는 일 |
|---|---|
| [질문](./question-agent.md) | 진입 판정, 교육 목록 확정과 설문 진행 |
| [구상](./plan-table-agent.md) | 계획표 뼈대 구성 — 회차·시행월·시간·강사·교육방법·평가방법과 회차별 담당 주제 배분 |
| [계획 작성](./plan-writer-agent.md) | 회차별 교육 내용 본문 작성과 생성 뒤 수정 |
| [요건 검사](./checker-agent.md) | 완성된 계획을 법정 기준·계획 내부 정합과 대조해 경고 |

이 순서 앞에는 발화를 가르는 [오케스트레이터](./orchestrator.md)가 있고, 순서에 단계로
들어가지 않는 에이전트가 둘 더 있습니다.

- **[편집 에이전트](./editor-agent.md)**는 담당자 발화를 결정 상태에 반영하는 부품입니다.
  질문·구상·계획 작성 세 에이전트가 **각자 호출**하므로 어느 단계에서 말하든 반영 방식이
  같습니다(요건 검사는 계획을 읽기만 하므로 호출하지 않습니다).
- **[조회 에이전트](./inquiry-agent.md)**는 정보를 묻는 발화만 처리하는 곁길입니다. 답한 뒤
  진행 중이던 단계의 화면이 다시 제시되며, 다음 단계로 넘기지 않습니다.

수정 요청은 어느 단계에서든 할 수 있고 **그 시점의 담당 에이전트가 받습니다** — 계획표를
보는 동안의 수정은 구상 에이전트가, 내용이 만들어진 뒤의 수정은 계획 작성 에이전트가
처리합니다. 내용 본문을 다시 쓸 수 있는 것은 계획 작성 에이전트뿐입니다.

## 시작 방식

탭에 진입하는 경로는 세 가지이며, 판정은 [질문 에이전트](./question-agent.md)가 합니다.
