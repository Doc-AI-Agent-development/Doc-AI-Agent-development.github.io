...
|---|---|
| 질문 | 교육 목록 확정과 설문 진행 |
| 구상 | 계획표 뼈대 구성 — 회차·시행월·시간·강사·교육방법·평가방법과 회차별 담당 주제 배분 |
| 계획 작성 | 회차별 교육 내용 본문 작성 |
| 요건 검사 | 완성된 계획을 법정 기준·계획 내부 정합과 대조해 경고 |

두 에이전트가 더 있으며, 이 순서에 단계로 들어가지 않습니다.

- **편집 에이전트**는 담당자 발화를 결정 상태에 반영하는 부품입니다. 질문·구상·계획 작성
  세 에이전트가 **각자 호출**하므로 어느 단계에서 말하든 반영 방식이 같습니다(요건 검사는
  계획을 읽기만 하므로 호출하지 않습니다). 반영 절차는
  [대화 반영 방식](./edits.md)에서 다룹니다.
- **조회 에이전트**는 정보를 묻는 발화만 처리하는 곁길입니다. 답한 뒤 진행 중이던 단계의
  화면이 다시 제시되며, 다음 단계로 넘기지 않습니다.

수정 요청은 어느 단계에서든 할 수 있고 **그 시점의 담당 에이전트가 받습니다** — 계획표를
보는 동안의 수정은 구상 에이전트가, 내용이 만들어진 뒤의 수정은 계획 작성 에이전트가
처리합니다. 내용 본문을 다시 쓸 수 있는 것은 계획 작성 에이전트뿐입니다. 에이전트마다
소유하는 결정과 읽고 쓰는 상태, 고칠 때 함께 봐야 하는 자리는
[연간 교육계획 에이전트 지도](../maintenance/annual-agents.md)에 정리되어 있습니다.
