...
각 팀이 실시해야 할 교육의 **실시 의무**와 각 교육의 **기준**을 [컴플라이언스 DB](../data/compliance-db.md)(관계형)에서 조회합니다. 벡터 검색이나 LLM 추론 없이 [`sql_query_tool`](../tools/sql-query-tool.md)로 정확 조회만 합니다.
...
- 벡터 검색·LLM 없이 [`sql_query_tool`](../tools/sql-query-tool.md)로 정확 조회만 합니다.
