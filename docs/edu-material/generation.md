...
교시별 지정 쪽수에서 고정 구성(표지·마무리)을 제외한 본문 예산을 산출하고, 주제 수로
나눈 몫을 각 주제의 작성 지시에 포함합니다. 생성 완료 후에는 실측 페이지 수와 지정
분량의 차이·방향(일치, 근접, 초과, 미달)을 **코드가 확정해 보고 발화에 공급**합니다([설계
원칙](../concepts/design-principles.md)의 사실 기반 컨텍스트 구성 참조). 분량이 지정보다 많거나
