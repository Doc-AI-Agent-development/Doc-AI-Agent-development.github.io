...
1. **스토리라인 작성** — 교시를 몇 개의 주제로 나눠 어떤 순서로 전개할지 설계합니다.
   ...
   학습목표와 **주제 시퀀스**를 만듭니다. 채택 문서는 발췌가 아니라 **전문**이 공급 분량
   한도 안에서 제공됩니다. 주제마다 제목과 역할, 핵심 메시지, 사용할 근거 문서와
   이미지가 배정됩니다. 교시를 닫는 사례·문답·요점 정리도 이 시퀀스의 정리 역할 주제로
   함께 설계되므로, 접수된 요구가 마무리에도 똑같이 반영됩니다. 페이지 경계는 여기서
   정하지 않습니다. 주제 시퀀스는 다음과 같은 모양입니다(가공 예시).
...
2. **이미지 선택** — 스토리라인이 주제에 배정한 이미지 후보를 LLM이 실물로 열어 보고
   판정합니다. 판정은 유지·이동·제외 세 가지입니다. 담당자가 교시당 사진 상한을 지정한
   경우 상한만큼만 남깁니다.
...
4. **교시 단위 정리** — 병렬 작성이 남긴 주제 간 중복과 경계를 넘은 내용을, 교시 전체
   유닛을 놓고 정리합니다. 아래 분량 처리 절의 분량 예산도 이 단계에서 반영합니다. 이
   호출이 실패하면 원본 유닛을 그대로 사용합니다.
