...
5. **발췌 추출** — 채택 문서에서 주제 키워드가 포함된 문단을 **원문 그대로** 추출합니다.
   발췌는 LLM 요약이 아니라 **원문에서 그대로 추출한 문단**입니다.
...
설정) 값을 그대로 담은 **대조용 기록**이 남습니다. 생성 시점에 현재 재료 값과 이 기록을
비교해, 하나라도 다르면 **생성을 차단하고 재수집을 안내**합니다. 낡았는지를 따로 저장해
두지 않고 매번 현재 값과 비교하므로, 대화가 몇 턴을 지나도 판정이 같습니다.
