...
| 생성 설정 | 문서 형태, 생성 단위, 난이도, 구성 방식, 요약본, 문서 수집 범위 |
...
<pre class="type-block"><code>class <span class="tname tname-genset">GenerationSettings</span>(BaseModel):
    ...
    evidence_scope: str | None   <span class="cmt"># 문서 수집 범위 — 첨부 문서만 / 사내 문서 보강</span>
...
값(교시 분량, 사진 상한, 문서 수집 범위)은 해석 없이 계획의 해당 자리에 그대로 기록됩니다.
