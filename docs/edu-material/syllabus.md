...
```mermaid
flowchart TD
  A["원본 조회와 보관<br/>(연간계획 행 · 법령 — 원본 불변)"] --> B["초안 작성<br/>(재료가 부족하면 사내 문서로 보강)"]
  B --> C["설정 카드 답 반영"]
...
**원본 조회와 보관.** 교육이 선택되면 백엔드에서 그 교육의 연간계획 행과 교육과정 정보,
법령 근거를 조회합니다. 이 원문이 구성 계획을 만드는 재료가 되며, 원문 자체는 별도로
보관되어 **이후 어떤 수정에도 변하지 않습니다**. 담당자의 수정은 다음 단계에서 만들어지는
구성 계획에만 쌓이므로, 원래 계획에서 무엇이 달라졌는지 언제든 원본과 대조할 수 있습니다.
즉석 주제나 첨부 문서 기반으로 시작하는 경우에는 그에 해당하는 재료가 대신 준비됩니다.

**초안 작성.** LLM이 준비된 재료로 교시 구성 초안을 작성합니다. 재료만으로 부족하면
...
**소비.** 확정된 계획은 근거 수집(교시별 주제를 검색 질의 재료로), 생성(설정과 교시
구성·쪽수), 시험(교시 구성과 요구사항), 화면(계획 블록)이 각자 읽습니다. 자료 생성
이후의 부분 수정은 **계획을 변경하지 않습니다** — 산출물 위의 수정은 "**무엇을
가르칠지**"의 변경이 아니기 때문입니다.

## 일관성이 유지되는 방식

계획이 여러 곳에서 읽히는데도 서로 어긋나지 않는 것은 두 가지 덕분입니다.

- **계획을 고치는 경로가 하나로 모입니다.** 카드 답 반영, 대화 수정, 수치 직접 지정 —
  어느 경로로 바꾸든 같은 계획 수정 코드를 거칩니다. 계획을 읽는 기능(근거 수집·생성·
  시험·화면)은 읽기만 하고 고치지 않으므로, 계획이 모르는 사이에 바뀌는 일이 없습니다.
- **모두가 같은 계획 하나를 읽습니다.** 담당자가 화면에서 보는 계획, 대화가 참조하는
  계획, 수정 요청을 해석할 때 기준이 되는 계획이 전부 같은 계획 객체입니다. 화면에
  보이는 것과 실제로 반영되는 것이 어긋나지 않는 이유입니다.
